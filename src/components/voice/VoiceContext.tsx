...
export const VoiceProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  // Directly process spoken or typed text. No other command runs while a
  // confirmation is pending; only an explicit confirm or cancel is accepted.
  const processSpokenPhrase = useCallback(
    (phrase: string): boolean => {
      ...
      setTimeout(() => {
        setState((prev) =>
          prev === "command-ambiguous" || prev === "command-unrecognized"
            ? "listening"
            : prev,
        );
      }, 3000);
